...
| **Yahoo data-gap detection** (2026-07-15) | `_is_normal_trading_gap()` | yfinance has repeatedly been observed to *silently skip* a real trading day for a given ticker (not error — just absent from the series), which silently turns a "1-day change" into a 2+ day change. Every ticker in the `tickers` loop and the CBOE loop now checks the calendar gap between its last two bars; an abnormal gap (not 1 day, not a Fri→Mon weekend) adds `"prev_date"` and `"gap_warning"` to that entry. The frontend (`updatePriceCard`, `renderUsIndices` in `js/app.js`) shows a `⚠` on the change badge with a tooltip when this fires. For **TWII specifically** (this dashboard's headline index), `_patch_twii_gap()` goes further: it fetches the missing day's real close from TWSE's own `MI_INDEX` report (authoritative, not an estimate) and inserts it into the series, so `gap_warning` clears and the real day-over-day move is shown instead of masked. Confirmed case: 2026-07-14 was missing from yfinance's `^TWII`/`^KS11`/`^N225`, hiding a real -1.42% TAIEX drop that day — TWII is now patched (44,737.95), KOSPI/Nikkei are flagged but not patched (no equivalent free authoritative source integrated yet). |
...
6. **Don't debug Chinese text with `python -c "print(...)"` piped through this Windows bash/git-bash terminal** — the console codepage mangles CJK output into `�` mojibake even when the underlying string/file is correct UTF-8. This produced a false "data corruption" scare on 2026-07-15 (institutional `title`, `signal.outlook`, heatmap stock names all looked garbled in terminal output but were byte-for-byte correct UTF-8 in the file). Use `grep`/a file-reading tool instead to verify Chinese text — they render correctly through this same terminal.
7. **A single `NaN` anywhere in `market_data.json` takes down the entire site** — Python's `json.dump()` defaults to `allow_nan=True` and happily writes a bare `NaN` token, which Python's own `json.load()` accepts but browsers' `JSON.parse()` rejects outright, so `fetch()` fails for the *whole* file, not just the offending field. This happened for real on 2026-07-15 (yfinance returned a NaN close for `^KS11`) and took the live site down until hotfixed. Two layers of defense now exist in `fetch_data.py`: (1) the `tickers` loop drops any row with a NaN `Close` before computing `current`/`prev`/MA stats, and (2) `main()` runs the final `output` dict through `_sanitize_nans()` (NaN/Inf → `null`) and writes with `allow_nan=False`, so anything this misses fails loudly in the Action log instead of silently deploying broken JSON. If a future run ever fails at the `json.dump` step, check for a new NaN source before assuming it's unrelated.
